...
describe("MetaContractOwnable", function () {
    ...
    const setup = async () => {
        ...
        const MetaContractOwnableFactory = await ethers.getContractFactory("MetaContractOwnable");
    ...
    it("should execute function on target contract by the owner", async function () {
        ...
        const result = await metaContract.connect(owner).executeFunction(targetContract.address, txData);
    ...
    it("should revert when executing function by a non-owner", async function () {
        ...
        await expect(metaContract.connect(other).executeFunction(targetContract.address, txData))
